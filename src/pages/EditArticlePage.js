...
// Enhanced image card component with caption functionality and drag-and-drop (matching CreateArticlePage)
...
const EditArticlePage = () => {
  ...
  const handleRemoveImage = async (imageId) => {
    try {
      ...
      if (imageToRemove) {
        // Try to delete from Cloudinary for both newly uploaded and existing images
        const publicIdToDelete = imageToRemove.publicId || imageToRemove.id;
        
        if (publicIdToDelete && (imageToRemove.status === 'completed' || !imageToRemove.status)) {
          console.log('🗑️ Deleting image from Cloudinary:', publicIdToDelete);
          try {
            await articleService.deleteImage(publicIdToDelete);
  ...
  const handleRemoveVideo = async (videoId) => {
    try {
      ...
      if (videoToRemove) {
        // Try to delete from Cloudinary for both newly uploaded and existing videos
        const publicIdToDelete = videoToRemove.publicId || videoToRemove.id;
        
        if (publicIdToDelete && (videoToRemove.status === 'completed' || !videoToRemove.status)) {
          console.log('🗑️ Deleting video from Cloudinary:', publicIdToDelete);
          try {
            await articleService.deleteVideo(publicIdToDelete);
  ...
  // Draggable wrapper component for images (matching CreateArticlePage)
